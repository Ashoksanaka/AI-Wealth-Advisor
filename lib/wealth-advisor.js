...
${PRODUCT_GUARDRAIL}
Support fractional/micro-investing — recommend starting with small amounts like ₹500.
When responding to chat, return JSON only:
{"text":"...","reasoning":"one sentence why","dataPoints":["specific data used"],"actions":[{"type":"link","label":"...","href":"..."},{"type":"invest_slider","label":"Invest now","min":500,"max":50000,"default":1000,"href":"/portfolio"},{"type":"goal_slider","label":"Boost goal contribution","min":500,"max":10000,"default":2000,"href":"/goals"}]}
...
function formatContextForPrompt(context) {
  ...
  const lines = [
    `Net Worth: ${formatINR(safe.netWorth, { decimals: 2 })}`,
    `This Month Income: ${formatINR(safe.spend.currentIncome, { decimals: 2 })}`,
    `This Month Expenses: ${formatINR(safe.spend.currentExpenses, { decimals: 2 })}`,
    `Savings Rate: ${safe.spend.savingsRate}%`,
    `Top Spending: ${safe.spend.topCategories.map((c) => `${c.category} ${formatINR(c.amount)}`).join(", ") || "none"}`,
    `Projected Month Surplus: ${formatINR(safe.cashFlow?.surplus ?? 0)}`,
  ];
  ...
  if (safe.budget) {
    lines.push(`Budget: ${formatINR(safe.budget.amount)} (${safe.budget.utilization?.toFixed(0) ?? 0}% used)`);
  }
  if (safe.riskProfile) {
    lines.push(
  ...
  if (safe.holdings.length > 0) {
    lines.push(
      `Holdings: ${safe.holdings.map((h) => `${h.name} (${formatINR(h.currentValue)})`).join(", ")}`
    );
  }
...
function enrichInsight(insight, context) {
  const dataPoints = [
    ...
    `Monthly expenses: ${formatINR(context.spend.currentExpenses)}`,
    `Net worth: ${formatINR(context.netWorth)}`,
  ];
  if (context.spend.topCategories[0]) {
    dataPoints.push(
      `Top category: ${context.spend.topCategories[0].category} (${formatINR(context.spend.topCategories[0].amount)})`
    );
  }
  if (context.cashFlow?.surplus) {
    dataPoints.push(`Projected surplus: ${formatINR(context.cashFlow.surplus)}`);
  }
...
function generateDashboardNarration(context) {
  ...
  let text = `Here's your financial snapshot. Net worth: ${formatINR(context.netWorth)}. You're saving ${context.spend.savingsRate}% of income this month, with expenses at ${formatINR(context.spend.currentExpenses)}.`;

  if (context.portfolioValue > 0) {
    text += ` Your portfolio is worth ${formatINR(context.portfolioValue)}`;
    if (context.portfolioGainLoss !== 0) {
      text += ` (${context.portfolioGainLoss >= 0 ? "+" : ""}${formatINR(context.portfolioGainLoss)} P&L)`;
    }
    text += ".";
  }
  ...
  if (topCategory) {
    text += ` Biggest spend category: ${topCategory.category} (${formatINR(topCategory.amount)}).`;
  }
...
function generateFallbackInsights(context) {
  ...
  if (context.spend.savingsRate < 10) {
    ...
    insights.push({
      ...
      dataPoints: [`Savings rate: ${context.spend.savingsRate}%`, `Projected surplus: ${formatINR(context.cashFlow?.surplus ?? 0)}`],
    });
  }
  ...
  if (context.cashFlow?.suggestedInvest > 0) {
    insights.push({
      ...
      summary: `Projected ${formatINR(context.cashFlow.surplus)} surplus — invest ${formatINR(context.cashFlow.suggestedInvest)} now.`,
      action: "Invest now",
      reasoning: "Cash flow prediction based on daily spend rate.",
      dataPoints: [`Projected surplus: ${formatINR(context.cashFlow.surplus)}`, `Suggested: ${formatINR(context.cashFlow.suggestedInvest)}`],
    });
  }
...
function generateEducationReply(message, context) {
  ...
  if (message.toLowerCase().includes("compound")) {
    return {
      text: `Think of compound interest like a snowball rolling downhill — your earnings generate more earnings. For ${hobby}, even ₹500/month can grow significantly over ${context.riskProfile?.investmentHorizonYears || 10} years.`,
      actions: [{ type: "link", label: "Invest ₹500", href: "/portfolio" }],
    };
  }
  return {
...
function generateFallbackReply(context, message) {
  ...
  let text = `Net worth: ${formatINR(context.netWorth)}, savings rate: ${context.spend.savingsRate}%.`;
  const actions = [];

  if (lower.includes("spending") || lower.includes("expense")) {
    text = `You've spent ${formatINR(context.spend.currentExpenses)} this month. Top category: ${context.spend.topCategories[0]?.category || "none"}.`;
    actions.push({ type: "link", label: "Review spending", href: "/dashboard" });
  } else if (lower.includes("invest") || lower.includes("portfolio")) {
    text = `Portfolio: ${formatINR(context.portfolioValue)}. I recommend YourBank approved products only.`;
    actions.push({ type: "link", label: "Invest ₹500", href: "/portfolio" });
...
function enrichAdvisorReply(reply, context, colloquialNotes = []) {
  const dataPoints = [
    ...
    `Savings rate: ${context.spend.savingsRate}%`,
    `Monthly expenses: ${formatINR(context.spend.currentExpenses)}`,
    `Projected surplus: ${formatINR(context.cashFlow?.surplus ?? 0)}`,
  ];
  if (context.spend.topCategories[0]) {
    dataPoints.push(
      `Top category: ${context.spend.topCategories[0].category} (${formatINR(context.spend.topCategories[0].amount)})`
  ...
  if (
    ...
    actions.push({
      ...
      label: "Invest now",
      min: 500,
      max: Math.max(50000, context.cashFlow?.suggestedInvest || 5000),
      default: Math.max(500, context.cashFlow?.suggestedInvest || 1000),
      href: "/portfolio",
    });
  }
