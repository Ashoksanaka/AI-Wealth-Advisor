...
export async function getNudgesForUser(userId) {
  ...
  if (summary.spend.currentIncome > 0) {
    ...
    nudges.push({
      ...
      message: `You received income this month. Move 10% (${formatINR(Math.round(salaryTx * 0.1))}) to investments?`,
      action: "Invest now",
      href: "/portfolio",
    });
  }

  if (summary.cashFlow.surplus > 5000) {
    nudges.push({
      id: "surplus",
      title: "Projected surplus",
      message: `You'll likely have ${formatINR(summary.cashFlow.surplus)} left this month. Invest ${formatINR(summary.cashFlow.suggestedInvest)}?`,
